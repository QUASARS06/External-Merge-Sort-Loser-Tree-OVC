...
int main(int argc, char *argv[])
{
	...
	printf("------------------------------ Test 11 --------------------------------\n");
	printf("Description : Many columns in each Row\n\n");
	
	// number of columns in each Row of Database Record
	int num_of_cols = argMap.find("-c") != argMap.end() ? std::atoi(argMap["-c"].c_str()) : 70;
	...
	int ram_capacity = argMap.find("-r") != argMap.end() ? std::atoi(argMap["-r"].c_str()) : 1000;

	// page_size = 20 means 1 page can store 20 records
	int page_size = argMap.find("-p") != argMap.end() ? std::atoi(argMap["-p"].c_str()) : 200;

	// Total number of Rows/Records to be generated
	int num_of_records = argMap.find("-n") != argMap.end() ? std::atoi(argMap["-n"].c_str()) : 20000;
